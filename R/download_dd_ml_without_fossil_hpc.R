...
download_dd_ml_without_fossil_rds_hpc <- function(job_ids) {
  ...
  # Files to download
  files <- glue::glue(
    paste0(path_to_fabrika_hpc(), "comrad_data/ml_results/dd_ml_without_fossil_{job_ids}.rds")
  )
...
#' @export
#' @rdname download_sim_hpc
download_dd_ml_without_fossil_log_hpc <- function(job_ids) {
  ...
  # Files to download
  files <- glue::glue(
    paste0(path_to_fabrika_hpc(), "comrad_data/logs/dd_ml_without_fossil_{job_ids}.log")
  )
  # Get ssh to download files
  purrr::walk(
    files,
