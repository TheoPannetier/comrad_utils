run_dd_ml_hpc_median <- function(siga, sigk, dd_model) {
  if (!is_on_peregrine()) {
  ...
  waiting_times_tbl <- comrad::summarise_waiting_times(waiting_times_tbl, median)
  ...
  saveRDS(
    ml,
    glue::glue(
      "/data/p282688/fabrika/comrad_data/ml_results/ml_{dd_model$name}_sigk_{sigk}_siga_{siga}_median.rds")
  )
}
